...
function printCoordinates() {
    ...
    for (var l = 0; l < b.lager.length; ++l) {
        ...
        liText  = current.name + ' at (' + current.getX() + ', ' + current.getY() + ')';
    ...
    for (var s = 0; s < b.stab.length; s++) {
        current = b.stab[s];
        li      = document.createElement('li');
        liText  = current.name + ' between ' + current.leftEnd + ' and ' + current.rightEnd;
        li.appendChild(document.createTextNode(liText));
...
function produceXml(bridge) {
    ...
    for (var l = 0; l < bridge.lager.length; l++) {
        details = createClosedXmlTag('details', {
            'mass': 0,
